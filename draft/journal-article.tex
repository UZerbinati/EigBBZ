...
\begin{theorem}
  ...
  \begin{equation}
    \abs{\omega-\omega_{h,i}}\leq C \max\{\rho_{1,s}(h),\rho_{2,r}(h)\}^2\max\{\norm{\phi_1}_{\vec{H}(\Omega)},\ldots,\norm{\phi_m}_{\vec{H}(\Omega)}\}^2, \qquad i=1,\ldots,m, 
  \end{equation}
...
\begin{proof}
  ...
  \begin{align}
    \abs{\Big((T-T_h)\phi_i,\phi_j\Big)} & \leq \abs{a((T-T_h)\phi_i,(T-T_h)\phi_j)} + C\norm{\vec{v}_h}_{\vec{V}} \Big(\rho_{2,r}(h)\norm{p_j}_{H^r(\Omega)} + \rho_{2,r}(h)\norm{p_i}_{H^r(\Omega)}\Big).
  \end{align}
...
\end{proof}
\uz{Since we decided to realte the keep FEEC considerations to the next section I decided to keep discussing everything in terms of the right bounded inverse here.
